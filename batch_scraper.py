#!/usr/bin/env python3
"""
安居客/58同城 办公室房源批量采集工具
====================================
用法:
  1. 把房源URL放进 urls.txt（一行一个）
  2. 运行: python batch_scraper.py
  3. 输出: projects.json（可直接导入 admin.html）

  也支持命令行直接传URL:
  python batch_scraper.py --url https://sz.sydc.anjuke.com/xzl-zu/7118047650/
  python batch_scraper.py --urls "https://.../A/ https://.../B/"
"""

import argparse
import json
import re
import sys
import os
import urllib.request
import urllib.error
import time
from urllib.parse import urlparse

# Windows 编码修复
if sys.platform == "win32":
    try:
        sys.stdout.reconfigure(encoding="utf-8", errors="replace")
        sys.stderr.reconfigure(encoding="utf-8", errors="replace")
    except Exception:
        pass

# ============================================================
# 配置
# ============================================================
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DEFAULT_URL_FILE = os.path.join(SCRIPT_DIR, "urls.txt")
DEFAULT_OUTPUT = os.path.join(SCRIPT_DIR, "projects.json")

MOBILE_UA = (
    "Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) "
    "AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.0 Mobile/15E148 Safari/604.1"
)

REQUEST_DELAY = 1.5  # 每条请求间隔秒数

AREAS = sorted(
    ["会展新城", "宝安中心", "南山科技园", "碧海湾", "前海", "西乡", "翻身", "新安", "兴东",
     "坪洲", "固戍", "福永", "沙井", "宝安", "南山", "福田", "罗湖", "龙华", "龙岗",
     "盐田", "光明", "坪山", "大鹏"],
    key=len, reverse=True  # 长优先匹配
)

# ============================================================
# 解析函数
# ============================================================

def fetch_page(url, timeout=20):
    """用移动端UA抓取页面"""
    req = urllib.request.Request(url)
    req.add_header("User-Agent", MOBILE_UA)
    req.add_header("Accept", "text/html,application/xhtml+xml")
    req.add_header("Accept-Language", "zh-CN,zh;q=0.9")
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            raw = resp.read()
            # 检测编码
            charset = "utf-8"
            ct = resp.getheader("Content-Type", "")
            m = re.search(r"charset=([\w-]+)", ct)
            if m:
                charset = m.group(1)
            return raw.decode(charset, errors="replace")
    except Exception as e:
        print(f"  ⚠️ 抓取失败: {e}")
        return ""


def strip_html(text):
    """去掉HTML标签，返回纯文本"""
    text = re.sub(r"<script[^>]*>[\s\S]*?</script>", " ", text, flags=re.IGNORECASE)
    text = re.sub(r"<style[^>]*>[\s\S]*?</style>", " ", text, flags=re.IGNORECASE)
    text = re.sub(r"<[^>]+>", " ", text)
    text = text.replace("&nbsp;", " ").replace("&lt;", "<").replace("&gt;", ">").replace("&amp;", "&")
    text = re.sub(r"\s+", " ", text)
    return text.strip()


def extract_title(html):
    """提取页面标题"""
    m = re.search(r"<title>([^<]+)</title>", html, re.IGNORECASE)
    if m:
        return m.group(1).strip()
    # 尝试 h1
    m = re.search(r"<h1[^>]*class=['\"]?bus-keytitle['\"]?[^>]*>([\s\S]*?)</h1>", html, re.IGNORECASE)
    if m:
        return strip_html(m.group(1))
    return ""


def guess_name(html, title, text):
    """从标题/文本猜项目名（多种策略）"""
    # 策略1: bus-map-title 锚点（最准确，如"前海卓越宝中时代广场"）
    m = re.search(r'<a[^>]*class="[^"]*bus-map-title[^"]*"[^>]*>\s*(.+?)\s*</a>', html, re.IGNORECASE)
    if m:
        name = strip_html(m.group(1))
        if len(name) >= 3 and "地图" not in name:
            return name

    # 策略2: 描述中的【楼盘】：或《1》项目: 或 项目:XXX
    m = re.search(r"(?:【楼盘】|《1》项目|项目)[：:]\s*(.+?)(?:<br|（|\n|<)", text)
    if m:
        name = m.group(1).strip()
        name = re.sub(r"（.*", "", name)  # 去掉括号备注
        if len(name) >= 3:
            return name

    # 策略3: h1 bus-keytitle 内容
    m = re.search(r"<h1[^>]*class=['\"]?bus-keytitle['\"]?[^>]*>([\s\S]*?)</h1>", html)
    if m:
        h1_text = strip_html(m.group(1))
        # 标题格式："出租丨宝安中心！壹方城丨壹方中心809平..." → 提取第二个丨之后
        parts = h1_text.split("丨")
        if len(parts) >= 2:
            # parts[1] = "宝安中心！壹方中心809平豪装..."
            second = parts[1]
            # 提取楼盘后缀词
            m2 = re.search(r".{1,30}?(大厦|大楼|中心|广场|写字楼|商务楼|办公楼|产业园|科技园|创业园|公寓|花园|城|馆|阁|苑|府|居|庭|座|壹方城|壹方中心|玖誉|卓越时代|卓越宝中)", second)
            if m2:
                return m2.group(0)
            # 取第一个!前面的内容作为区域+项目线索
            first_bang = second.split("！")[0] if "！" in second else second[:20]
            return first_bang[:30]

    # 策略4: 常用楼盘后缀（在全文文本中搜索）
    suffix_patterns = [
        r"前海[^，,\s]{2,10}(?:大厦|中心|广场|时代|城|府|馆|阁|座)",
        r"([^\s，,]{2,20}(?:大厦|大楼|中心|广场|写字楼|商务楼|办公楼|产业园|科技园|创业园|公寓|花园|城|馆|阁|苑|府|居|庭|座|壹方城|壹方中心|玖誉))",
    ]
    for pat in suffix_patterns:
        m = re.search(pat, text)
        if m:
            name = m.group(1) if m.lastindex else m.group(0)
            if len(name) >= 3 and name not in ("科技园", "创业园", "写字楼", "办公楼"):
                return name

    # 策略5: title 兜底
    clean = re.sub(r"[【】出租办公多图低价急租房东直租业主一手看海景地铁口]", "", title)
    clean = clean.strip()[:30]
    return clean if len(clean) >= 4 else "未知项目"


def guess_area(html):
    """提取区域"""
    # 方式1: <text>宝安</text> ... <text> · 宝安中心区</text>
    m = re.search(r"<text>\s*(宝安|南山|福田|罗湖|龙华|龙岗|盐田|光明|坪山|大鹏)\s*</text>", html)
    district = m.group(1) if m else ""

    sub_m = re.search(r"<text>\s*·\s*(\S+?)\s*</text>", html)
    sub = sub_m.group(1) if sub_m else ""

    # 方式2: <div class="bus-map-name">
    m2 = re.search(r'<div[^>]*class="[^"]*bus-map-name[^"]*"[^>]*>\s*(.+?)\s*</div>', html)
    if m2:
        loc = strip_html(m2.group(1))
        return loc

    # 方式3: 在 title 中匹配
    title = extract_title(html)
    for a in AREAS:
        if a in title:
            return a

    if district:
        return f"{district}-{sub}" if sub else district
    return ""


def guess_location(html):
    """提取详细地址"""
    # 策略1: bus-map-title 锚点（最准确的项目地址）
    m = re.search(r'<a[^>]*class="[^"]*bus-map-title[^"]*"[^>]*>\s*(.+?)\s*</a>', html, re.IGNORECASE)
    if m:
        addr = strip_html(m.group(1))
        # 用bus-map-title作为项目名，地址从描述中提取
        pass  # fall through to other strategies

    # 策略2: QA中楼盘地址
    m = re.search(r"楼盘地址[：:]\s*([^<]+)", html)
    if m:
        return m.group(1).strip()

    # 策略3: <div class="bus-map-name">宝安-宝安中心区</div>
    m = re.search(r'<div[^>]*class="[^"]*bus-map-name[^"]*"[^>]*>\s*(.+?)\s*</div>', html)
    if m:
        return strip_html(m.group(1))

    return ""


def guess_size(html):
    """提取面积（多种策略）"""
    text = strip_html(html)

    # 策略1: 描述中的《2》面积:150㎡ 或 【面积】:809平
    m = re.search(r"(?:《2》面积|[【\[]面积[】\]])\s*[：:]\s*(\d+)\s*[平㎡mM]", text)
    if m:
        return m.group(1) + "㎡"

    # 策略2: meta description "面积800平米"
    m = re.search(r'<meta[^>]*name="description"[^>]*content="[^"]*面积\s*(\d+)\s*[平米㎡]', html)
    if m:
        return m.group(1) + "㎡"

    # 策略3: "XXX平" 模式（描述中比标题更可靠）
    m = re.search(r"(\d+)\s*[平㎡]", text)
    if m:
        # 过滤像"4.5米"、"5052"（车位）这样的数字
        num = int(m.group(1))
        if 30 < num < 10000:  # 合理面积范围
            return m.group(1) + "㎡"

    # 策略4: 标题中提取
    title = extract_title(html)
    m = re.search(r"(\d+)\s*[平㎡mM]", title)
    if m:
        num = int(m.group(1))
        if 30 < num < 10000:
            return m.group(1) + "㎡"

    return ""


def guess_price(html):
    """提取价格（多种策略）- 只返回元/㎡/月格式"""
    text = strip_html(html)

    # 策略1: meta description "单价2.2元/㎡/天" → 转换为月单价
    m = re.search(r'<meta[^>]*name="description"[^>]*content="[^"]*单价\s*([\d.]+)\s*元\s*/\s*㎡\s*/\s*天', html)
    if m:
        daily = float(m.group(1))
        monthly = int(daily * 30)
        return f"{monthly}元/㎡/月"

    # 策略2: 描述中的 【单价】：66元/平 或 《3》租金:65元/平
    m = re.search(r"(?:【单价】|《3》租金|租金)[：:]\s*([\d.]+)\s*元\s*/\s*平", text)
    if m:
        return f"{m.group(1)}元/㎡/月"

    # 策略3: price div（安居客移动版）"2.2元/㎡/天" → 转换为月单价
    m = re.search(r'<div[^>]*class="[^"]*price[^"]*"[^>]*>\s*([\d.]+)\s*<span[^>]*class="[^"]*unit[^"]*"[^>]*>\s*元\s*/\s*㎡\s*/\s*天', html)
    if m:
        daily = float(m.group(1))
        monthly = int(daily * 30)
        return f"{monthly}元/㎡/月"
    
    # 策略4: "65元/㎡/月"
    m = re.search(r"([\d.]+)\s*元\s*/\s*[㎡平米]\s*/\s*月", text)
    if m:
        return f"{m.group(1)}元/㎡/月"

    # 策略5: "65元/平"
    m = re.search(r"([\d.]+)\s*元\s*/\s*平", text)
    if m:
        return f"{m.group(1)}元/㎡/月"

    # 策略6: "10000元/月" - 需要面积才能计算单价，这里先返回原始格式
    m = re.search(r"([\d.]+)\s*元\s*/\s*月", text)
    if m:
        return f"{m.group(1)}元/月"

    return ""


def guess_decoration(html):
    """装修"""
    text = strip_html(html)
    if "豪华" in text or "豪装" in text:
        return "豪华装修"
    if "精装" in text:
        return "精装修"
    if "简装" in text:
        return "简装"
    if "毛坯" in text:
        return "毛坯"
    return "精装修"


def guess_floor(html):
    """楼层"""
    text = strip_html(html)
    # "中区/共49层"
    m = re.search(r"([低中高]区).*?共\s*(\d+)\s*层", text)
    if m:
        return f"{m.group(1)} / 共{m.group(2)}层"
    m = re.search(r"([低中高]区)", text)
    if m:
        return m.group(1)
    m = re.search(r"共\s*(\d+)\s*层", text)
    if m:
        return f"共{m.group(1)}层"
    return ""


def guess_features(html):
    """配套/标签"""
    text = strip_html(html)
    kw = {
        "近地铁": "近地铁" in text,
        "地铁口": "地铁口" in text,
        "配套食堂": bool(re.search(r"食堂|餐饮|餐厅", text)),
        "停车位": bool(re.search(r"停车|车位", text)),
        "海景": "海景" in text,
        "可注册": "可注册" in text,
        "红本": "红本" in text,
        "24小时空调": "24小时空调" in text or "独立控制" in text,
        "拎包入驻": "拎包" in text or "随时入住" in text or "随时入驻" in text,
        "配家私": "家私" in text or "家具" in text or "配家私" in text,
        "采光好": "采光" in text,
        "户型方正": "户型方正" in text,
        "5A甲级": "5A" in text or "甲级" in text,
        "中央空调": "中央空调" in text,
        "交通便捷": "交通" in text,
    }
    return [k for k, v in kw.items() if v]


def guess_image(html):
    """提取房源图片（安居客 swiper 图集第一张）"""
    # 策略1: swiper 第一张 fullview（最准确的房源主图）
    m = re.search(r'data-background="(https?://pic\d+\.ajkimg\.com/display/hj/[a-f0-9]+/600x450c\.jpg)\?', html)
    if m:
        # 换用更高分辨率版本
        return m.group(1).replace("600x450c", "800x600c")

    # 策略2: swiper 任意一张
    m = re.search(r'data-background="(https?://pic\d+\.ajkimg\.com/display/[^"]+?\.jpg)\?', html)
    if m:
        return m.group(1).replace("600x450c", "800x600c")

    # 策略3: swiper img src
    m = re.search(r'<img[^>]*src="(https?://[^"]+?sydccover[^"]+?\.jpg)[^"]*"', html)
    if m:
        return m.group(1)

    return ""


def parse_page(html, url):
    """解析单个页面，返回房源dict"""
    title = extract_title(html)
    text = strip_html(html)

    if not title or len(title) < 5:
        print(f"  ⚠️ 页面内容异常（可能是验证码/空页面）")
        return None

    name = guess_name(html, title, text)
    area = guess_area(html)
    location = guess_location(html)
    size = guess_size(html)
    price = guess_price(html)
    decoration = guess_decoration(html)
    floor = guess_floor(html)
    features = guess_features(html)
    image = guess_image(html)

    return {
        "name": name,
        "area": area,
        "location": location,
        "size": size,
        "price": price,
        "decoration": decoration,
        "floor": floor,
        "features": features,
        "image": image,
        "date": datetime.date.today().isoformat(),
    }


def clean_url(url):
    """简化安居客URL，去掉追踪参数"""
    url = url.strip()
    if not url:
        return ""

    # 安居客: https://sz.sydc.anjuke.com/xzl-zu/7118047650/?...
    m = re.match(r"(https?://[^/]+/(?:xzl-zu|cw-zu)/\d+/)", url)
    if m:
        return m.group(1)

    # 58同城: https://sz.58.com/...
    m = re.match(r"(https?://[^/]+/[\w-]+/[\w-]+/[\w./?]*)", url)
    if m:
        return m.group(1)

    return url


def read_url_file(filepath):
    """从文件读取URL列表（每行一个）"""
    urls = []
    with open(filepath, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if line and not line.startswith("#"):
                clean = clean_url(line)
                if clean:
                    urls.append(clean)
    return urls


def save_json(data, filepath):
    """保存JSON文件"""
    with open(filepath, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    print(f"\n✅ 已保存 {len(data)} 条房源到: {filepath}")


def print_summary(projects):
    """打印结果摘要"""
    print("\n" + "=" * 100)
    print(f"{'#':<4} {'项目名称':<28} {'区域':<14} {'面积':<14} {'价格':<22} {'装修'}")
    print("-" * 100)
    for i, p in enumerate(projects, 1):
        name = p["name"][:26] if p["name"] else "—"
        area = p["area"][:12] if p["area"] else "—"
        size = p["size"][:12] if p["size"] else "—"
        price = p["price"][:20] if p["price"] else "—"
        deco = p["decoration"] if p["decoration"] else "—"
        print(f"{i:<4} {name:<28} {area:<14} {size:<14} {price:<22} {deco}")
    print("-" * 100)
    print(f"  共 {len(projects)} 条房源")
    print("=" * 100)


# ============================================================
# 主入口
# ============================================================

def main():
    parser = argparse.ArgumentParser(
        description="安居客/58同城办公室房源批量采集",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  从文件读取:  python batch_scraper.py
  指定文件:    python batch_scraper.py --input my_urls.txt
  单个URL:     python batch_scraper.py --url https://sz.sydc.anjuke.com/xzl-zu/7118047650/
  多个URL:     python batch_scraper.py --urls "https://.../A/ https://.../B/"
  指定输出:    python batch_scraper.py --output my_projects.json
        """,
    )
    parser.add_argument("--input", "-i", help="URL列表文件路径 (默认: urls.txt)")
    parser.add_argument("--url", "-u", help="单个房源URL")
    parser.add_argument("--urls", help="多个URL，空格分隔，用引号包裹")
    parser.add_argument("--output", "-o", default=DEFAULT_OUTPUT, help="输出JSON文件路径")
    parser.add_argument("--no-delay", action="store_true", help="跳过请求间隔")
    args = parser.parse_args()

    # 收集URL
    urls = []
    if args.url:
        urls.append(clean_url(args.url))
    if args.urls:
        for u in args.urls.split():
            u = u.strip()
            if u:
                urls.append(clean_url(u))
    if not urls:
        url_file = args.input or DEFAULT_URL_FILE
        if not os.path.exists(url_file):
            print(f"❌ URL文件不存在: {url_file}")
            print(f'   请创建 {url_file}，每行一个房源URL，或使用 --url 参数')
            sys.exit(1)
        urls = read_url_file(url_file)

    if not urls:
        print("❌ 没有找到有效的URL")
        sys.exit(1)

    print(f"\n🔍 准备解析 {len(urls)} 条房源...")
    print(f"📁 输出文件: {args.output}\n")

    projects = []
    success = 0
    failed = 0

    for i, url in enumerate(urls, 1):
        print(f"[{i}/{len(urls)}] {url}")
        html = fetch_page(url)
        if not html:
            failed += 1
            continue

        proj = parse_page(html, url)
        if proj:
            projects.append(proj)
            success += 1
            print(f"  ✅ {proj['name']} | {proj['area']} | {proj['size']} | {proj['price']}")
        else:
            failed += 1
            print(f"  ⚠️ 未能解析")

        if i < len(urls) and not args.no_delay:
            time.sleep(REQUEST_DELAY)

    # 输出
    print(f"\n📊 完成: {success} 成功, {failed} 失败")
    if projects:
        print_summary(projects)
        save_json(projects, args.output)
        print(f"\n💡 下一步: 打开 admin.html → 点「📥 导入JSON」→ 粘贴 {args.output} 的内容")


if __name__ == "__main__":
    main()
